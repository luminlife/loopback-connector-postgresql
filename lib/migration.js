...
function mixinMigration(PostgreSQL) {
  ...
  PostgreSQL.prototype.addIndexes = function(model, actualIndexes, cb) {
    ...
    var indexNames = m.settings.indexes && Object.keys(m.settings.indexes).filter(function(name) {
      /*
       * Skip any model-defined indexes that we want Loopback to ignore
       * when determining what indexes to refresh
       */
      var index = m.settings.indexes[name];
      return !!(index && (!index.options || !index.options.skipExistenceCheck)) ;
    }) || [];
    var sql = [];
    var ai = {};
    ...
    if (actualIndexes) {
      actualIndexes.forEach(function(i) {
        ...
        /*
         * Skip any existing DB indexes that we want Loopback to ignore
         * when determining what indexes to refresh
         */
        if (m.settings.indexes && m.settings.indexes[name]) {
          var options = m.settings.indexes[name].options;

          if (options && options.skipExistenceCheck) {
            return;
          }
        }
